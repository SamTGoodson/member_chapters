...
plt.title("White Transplant Ratio Level by ED")
plt.show()
```
...
#| warning: false
#| fig-cap: 'Table 1'
a_tbl = cluster_df.groupby('cluster')[['mhhi21', 'nhb21p', 'nhw21p','cvap21bapp','w2admp','w2cmp','w2mgtp','w2pservep','w2salep','w2oadp','w2cossp']].mean().round(2).reset_index()
a_tbl = a_tbl.rename(columns={'mhhi21':'MHI','cvap21bapp':'BA+','nhb21p':"NH Black",'nhw21p':'NH White','w2admp':'Artists, Designers, and Journalists','w2cmp':'Software Engineers','w2mgtp':'Managment','w2pservep':'Service',"w2salep":'Sales','w2oadp':'Office Administration'
})
...
plt.title('Difference in Individual Contributions')
plt.show()
```
...
md_table = tabulate(corr_matrix, headers='keys', tablefmt='pipe')
...
quick_compare['assembley'] = quick_compare['assembley'].map(assembly_mapping)
tabl = quick_compare[['ed_name','assembley','council','candidate']].groupby(['assembley','council','candidate']).count()
tabl_columns = tabl.columns
Markdown(tabulate(tabl))
...
b_tbl_cols = b_tbl.columns
Markdown(tabulate(b_tbl,headers = b_tbl_cols, showindex = False))
```

This demographic breakdown offers a fascinating look at DSA support. The difference in all variables shown above is significant at the 99.9% confidence interval, meaning the chance that the demographic variation between these groups is random is practically 0. Categories 1 and 2, which vote for the DSA some or all of the time are whiter, better educated, higher income, and more likely to be artists, journalists, or designers than category zero.  As discussed in Chapter 2 this last occupation category is a key indicator for DSA sucess. This aligns with stereotypes about the DSA citywide, as well as Hendrickson's estimation that the organizatin could be doing better with working-class voters of color. In this election, however, Hudson was able to break up the DSA coalition and take the whitest, highest educated, and wealthiest EDs into her coalition. While both categories are likely to be ADM, the category that broke with Hollingsworth is significantly less likely to be office clerks or admin. The core of the DSA electoral coalition, from this sample, seems be highly educated but middle income whites living in along Franklin Ave. The wealthier whites in Prospect Heights may be happy to accept a DSA candidate in races such as Brisport's and Souffrant's, which is to say there may be no particular DSA stigma there, but are also eager to jump to a more traditional progressive candidate when one comes along. Especially one endorsed by local and national Democratic leadership. Hudson managed to artfully thread this coalition of Hasidic voters worried about Isreal and high-income voters looking for a traditional progressive and win. This is a real challenge to the DSA outsider strategy metioned above.

## Conclusion

The 35th council district is a diverse district, representing many of the biggest changes to NYC demographics and politics. It features large white and Black population, with many in-group differences in those larger racial categories. It features newcomers and long-time residents, and it has a large spectrum of income and education. Crystal Hudson's winning coalition managed to win completely the large Hasidic bloc in the southeast of the district, as well as pick several high-income, highly educated whites off of the DSA coalition in the area just north of Grand Army plaza. This has important implications for the process of coaltion formation. First, it offers some evidence for differential demographic support for certain types of progressives. High-income liberal whites might be counted on to join a progressive coalition, but with a preference for more traidtion, party endorsed Democrats, whereas highly educated middle-income whites may be the ideal demographic pocket for further left progressives. Second, it is a reminder of the importance of conservative blocs. If the Hasidic self-reported numbers are close to correc, Holllingsworth could have kept the entire Brisport and Suffrant coaltion intact and still lost the race, almost entirely because of his stance on Isreal (which, to clearly state the obvious, the New York City Council has no control over). Even a reduction in fervor over his stances on this issue might have reduced turnout and put him closer to the council. This offers a difficult picture for the DSA's twin stances of ideoligcal purity and outsider campagin status. Finally, it begs questions surrounding how these previous matters affect actual policy. How much does it matter to the proudction of progressive policy that the insider with more wealthy support won? To those in the DSA section of the coalition it matters quite a lot.